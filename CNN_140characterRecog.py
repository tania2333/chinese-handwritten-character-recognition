# -*- coding: utf-8 -*-
"""
Created on Tue Nov 13 20:09:20 2018
...
def read_from_gnt_dir(gnt_dir=train_data_dir):
    def one_file(f):                                        ## gnt文件由header和image数据两部分构成，header由10个字节构成，见note
        header_size = 10
        while True:
    ...
    for file_name in os.listdir(gnt_dir):
        if file_name.endswith('.gnt'):
            ...
            with open(file_path, 'rb') as f:                    ##“rb” 以”二进制 读”模式打开
...
def resize_and_normalize_image(img):
    # 补方
    pad_size = abs(img.shape[0] - img.shape[1]) // 2                  #[0]宽度，[1]高度， //:返回不大于结果的一个最大的整数
    if img.shape[0] < img.shape[1]:
        pad_dims = ((pad_size, pad_size), (0, 0))
    else:
        pad_dims = ((0, 0), (pad_size, pad_size))
    img = np.lib.pad(img, pad_dims, mode='constant', constant_values=255)            #img上下分别补0行，前后分别补pad_size列
    # 缩放
    img = scipy.misc.imresize(img, (64 - 4 * 2, 64 - 4 * 2))                          #调整图片大小,并将像素值调整到（0，255)范围内
    img = np.lib.pad(img, ((4, 4), (4, 4)), mode='constant', constant_values=255)    #保证汉字在中间，以便允许训练过程中的形变可以有空间进行
    assert img.shape == (64, 64)                                                      #声明其布尔值必须为真的判定，若其返回值为假，就会触发异常

    img = img.flatten()                                                                #把二维矩阵变为一维数组
    # 像素值范围-1到1
    img = (img - 128) / 128
    return img
...
# one hot
def convert_to_one_hot(char):                                                          #某个汉字出现时，将其在vector里对应的位置标记为1.
...
for image, tagcode in read_from_gnt_dir(gnt_dir=train_data_dir):
    tagcode_unicode = struct.pack('>H', tagcode).decode('gb2312')                     #tagcode解码得出对应汉字
    if tagcode_unicode in char_set:
        train_data_x.append(resize_and_normalize_image(image))                          #append:在列表末尾添加新的对象
        train_data_y.append(convert_to_one_hot(tagcode_unicode))

# shuffle样本
train_data_x, train_data_y = shuffle(train_data_x, train_data_y, random_state=0)        #shuffle() 方法将序列的所有元素随机排序
        ...
        for e in range(50):
            for i in range(num_batch):
                ...
                if (e * num_batch + i) % 100 == 0:
                    # 计算准确率
                    acc = accuracy.eval({X: test_data_x[:500], Y: test_data_y[:500], keep_prob: 1.})
                    # acc = sess.run(accuracy, feed_dict={X: test_data_x[:500], Y: test_data_y[:500], keep_prob: 1.})
